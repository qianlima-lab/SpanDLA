...
def train(model, args, epoch, dataset, logger, optimizer):
    ...
    with tqdm(desc='Training', total=len(dataset)) as pbar:
        for i, (data, adj, target, paths) in enumerate(dataset):
            if True:
                pbar.update()
                model.zero_grad()
                ...
                output, loss = model(data, adj, targets)
                loss.backward()
                optimizer.step()
...
def validate(model, args, epoch, dataset, logger):
    ...
    with tqdm(desc='Validatinging', total=len(dataset)) as pbar:
        for i, (data, adj, target, paths) in enumerate(dataset):
            if True:
                ...
                targets = (maybe_cuda(torch.tensor(seg_target).long()), maybe_cuda(torch.tensor(class_target).long()),
                           maybe_cuda(torch.tensor(combine_target).long()))
                output, loss = model(data, adj, targets)
                output_seg = F.softmax(output[0], 1).data.cpu().numpy().argmax(axis=1)
                output_sc_softmax = F.softmax(output[1], 1)
                output_sc = output_sc_softmax.data.cpu().numpy().argmax(axis=1)
                combine_seg = utils.transform2seg(output_sc)
                preds_stats.add(combine_seg, combine_target)
                seg_all_pk.append(utils.calculate_pk(output_seg, seg_target))
                all_pk.append(utils.calculate_pk(combine_seg, combine_target))

                f1, map = utils.get_section_level_evl(combine_target, output_sc_softmax.data.cpu().numpy(),
                                                      combine_seg)
                all_micro_f1.append(f1)
                all_map.append(map)
                all_acc.append(preds_stats.get_accuracy())
                pbar.set_description('Validatinging, loss={:.4}'.format(loss.item()))
            # except Exception as e:
            #     # logger.info('Exception "%s" in batch %s', e, i)
            #     logger.debug('Exception while handling batch with file paths: %s', paths, exc_info=True)
            #     pass
        ...
        logger.info(
            'Validating Epoch: {} ,accuracy: {:.4},segpk: {:.4} Pk: {:.4},Micro-F1: {:.4},MAP: {:.4} . '.format(
                epoch + 1,
                np.mean(all_acc),
                np.mean(seg_all_pk),
                np.mean(all_pk),
...
def main(args):
    ...
    train_dataset = RefDataset(data_path + '/train')
