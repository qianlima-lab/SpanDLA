...
class HANNModel(BaseModel):
    ...
    def add_logits_op(self, word_embeddings, sentence_lengths, document_lengths, dropout):
        ...
        if self.config.use_cnn:
                    ...
                    if self.config.use_cnn_rnn:
                        ...
                        if self.config.use_attention:
                            ...
                            A = tf.transpose(tf.reshape(tf.matmul(U_sent, U_word), shape=[-1, s[2],
                                             ...
                                             perm=[0, 2, 1])  # (bz, attn_hop, len)
                            A += 100000. * (tf.tile(
                                tf.expand_dims(tf.cast(tf.sequence_mask(sentence_lengths), tf.float32), axis=1),
            ...
            if self.config.use_cnn_rnn:
                ...
                cnn_filter_tot_num = len(
                    self.config.cnn_filter_sizes) * self.config.cnn_filter_num * self.config.attention_hop
            ...
            if self.config.use_attention:
                with tf.variable_scope("bi-lstm-sentence", reuse=tf.AUTO_REUSE):
                    ...
                    A = tf.transpose(tf.reshape(tf.matmul(U_sent, U_word), shape=[-1, s[2], self.config.attention_hop]),
                                     perm=[0, 2, 1])  # (bz, attn_hop, len)
                    A += 100000. * (
                            tf.tile(tf.expand_dims(tf.cast(tf.sequence_mask(sentence_lengths), tf.float32), axis=1),
                                    [1, self.config.attention_hop, 1]) - 1)
                    self.A = tf.nn.softmax(A)  # (bz, attn_hop, len)
                    output = tf.reshape(output, shape=[-1, s[2],
                                                       2 * self.config.hidden_size_lstm_sentence])  # (bz, len, hidden_size)
            ...
            if self.config.use_document_level == True:
                output = tf.reshape(output,
                                    [-1, s[1], self.config.attention_hop * 2 * self.config.hidden_size_lstm_sentence])
    ...
    def add_loss_op(self, logits, logits_no_dropout, labels, document_lengths):
        ...
        if self.config.use_crf:
            ...
            self.trans_params_sc = trans_params  # need to evaluate it for decoding
    ...
    def build(self):
        ...
        if self.config.drop_penalty > 0.:
            self.logits_no_dropout = self.forward(self.word_ids, self.char_ids,
                                                  self.word_lengths,
                                                  self.sentence_lengths,
                                                  self.document_lengths, 1.0)
        else:
            self.logits_no_dropout = None

        self.add_pred_op()
        with tf.variable_scope("sc", reuse=tf.AUTO_REUSE):
            self.loss_sc = self.add_loss_op(self.logits, self.logits_no_dropout,
                                            self.labels, self.document_lengths)
        self.loss = self.loss_sc
    ...
    def predict_batch(self, words):
        ...
        if self.config.use_crf:
            ...
            logits, trans_params = self.sess.run(
                [self.logits, self.trans_params_sc], feed_dict=fd)
    ...
    def run_evaluate(self, test, report=True):
        ...
        for words, labels in minibatches(test, self.config.batch_size):
            ...
            for lab, lab_pred, length in zip(labels, labels_pred,
                ...
                pk.append(calculate_seg_eval(((transform2seg(np.array(lab_pred) + 1))>0).astype(int)
                                             , ((transform2seg(np.array(lab) + 1))>0).astype(int)))

        labs = [self.idx_to_tag[lab].split('_')[0] for lab in labs]
        labs_pred = [self.idx_to_tag[lab_pred].split('_')[0] for lab_pred in labs_pred]
        labs_idx = [self.config.vocab_tags[l] + 1 for l in labs]
        labs_pred_idx = [self.config.vocab_tags[l] + 1 for l in labs_pred]
        ...
        tp, fn = get_seq_eval(transform2seg(labs_idx), transform2seg(labs_pred_idx))
        pk = np.mean(pk)
        unit = np.sum(tp) / (np.sum(tp) + np.sum(fn))
        ...
        return {"acc": 100 * acc, "macro-f1": 100 * macro_f1, "micro-f1": 100 * micro_f1,
                ...
                "confusion-matrix": confusion, 'span-f1': 100 * span_f1, 'pk': 100 * pk}
