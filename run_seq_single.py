    ...
    with tqdm(desc='Validatinging', total=len(dataset)) as pbar:
        for i, (data, adj, target, raw_sentences, paths) in enumerate(dataset):
            if True:
                ...
                all_pred.extend(output_seg)
                all_glod.extend(class_target)
        ...
        sentence_f1, span_f1 = utils.get_seq_eval(utils.transform2seg(np.array(all_glod)),
                                                  utils.transform2seg(np.array(all_pred)))
        logger.info(
