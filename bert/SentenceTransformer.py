...
class SentenceTransformer(nn.Sequential):
    ...
    def encode(self, sentences: Union[str, List[str], List[int]],
        ...
        inp_dataloader = DataLoader(inp_dataset, batch_size=batch_size,
                                    collate_fn=self.smart_batching_collate_text_only, num_workers=num_workers,
                                    shuffle=False)
    ...
    def start_multi_process_pool(self, target_devices: List[str] = None, encode_batch_size: int = 32):
        ...
        for cuda_id in target_devices:
            p = ctx.Process(target=SentenceTransformer._encode_multi_process_worker,
                            args=(cuda_id, self, input_queue, output_queue, encode_batch_size), daemon=True)
            p.start()
            processes.append(p)
    ...
    def _encode_multi_process_worker(target_device: str, model, input_queue, results_queue, encode_batch_size):
        ...
        while True:
            try:
                ...
                embeddings = model.encode(sentences, device=target_device, is_pretokenized=is_pretokenized,
                                          show_progress_bar=False, convert_to_numpy=True, batch_size=encode_batch_size)
    ...
    def save(self, path):
        ...
        if path is None:
            return
        path="output/scibert-mean-tokens-" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
