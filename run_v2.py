    ...
    with tqdm(desc='Validatinging', total=len(dataset)) as pbar:
        for i, (data, adj, target, paths) in enumerate(dataset):
            if True:
                ...
                all_pk.append(utils.calculate_pk(output_seg, combine_target))

                f1, map = utils.get_section_level_evl(torch.cat(target, 0).numpy(), output_softmax.data.cpu().numpy(),
                                                      output_seg)
        ...
        logger.info(
            'Validating Epoch: {} ,accuracy: {:.4}, Pk: {:.4},Micro-F1: {:.4},MAP: {:.4} . '.format(
                epoch + 1,
                np.mean(all_acc),
                np.mean(all_pk),
