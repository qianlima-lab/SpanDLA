    ...
    with tqdm(desc='Validatinging', total=len(dataset)) as pbar:
        for i, (data, adj, target, raw_sentences, paths) in enumerate(dataset):
            if True:
                ...
                output_seg = utils.transform2seg(output_softmax.data.cpu().numpy().argmax(axis=1))
                de_output_softmax = F.softmax(de_output, 1)
                de_output_seg = de_output_softmax.data.cpu().numpy().argmax(axis=1)
                ...
                com_gold.extend(combine_target.tolist())
        ...
        de_sentence_f1, de_span_f1 = utils.get_seq_eval(np.array(com_gold), utils.transform2seg(np.array(de_pred)))
        logger.info(
            'Validating Epoch: {} ,accuracy: {:.4}, Pk: {:.4},windiff:{:.4},Micro-F1: {:.4},MAP: {:.4},sentence-f1 {:.4},span-f1 {:.4},de-sentence-f1 {:.4},de-span-f1 {:.4} '.format(
                epoch + 1,
                np.mean(all_acc),
                np.mean(all_pk),
                np.mean(all_windiff),
