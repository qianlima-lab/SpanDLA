    ...
    with tqdm(desc='Training', total=len(dataset)) as pbar:
        for i, (data, adj, target, paths) in enumerate(dataset):
            if True:
                ...
                pred_seg, pred_sc_output, seg_sc_gold, loss = model(data, epoch, targets)
                loss.backward()
                optimizer.step()
        ...
        for i, (data, adj, target, paths) in enumerate(dataset):
            if True:
                ...
                targets = (maybe_cuda(torch.tensor(seg_target).long()), maybe_cuda(torch.tensor(class_target).long()),
                           maybe_cuda(torch.tensor(combine_target).long()))
                pred_seg, pred_sc_output, seg_sc_gold, loss = model(data, epoch, targets)
                output_softmax = F.softmax(pred_sc_output, 1)
                output_sc = output_softmax.data.cpu().numpy().argmax(axis=1)
                preds_stats.add(output_sc, seg_sc_gold)
                pk, windiff = utils.calculate_seg_eval(pred_seg.cpu().numpy(), seg_target)
                all_pk.append(pk)
                all_windiff.append(windiff)
                f1, map = utils.get_section_level_evl(seg_sc_gold, output_softmax.data.cpu().numpy(),
                                                      output_sc)
